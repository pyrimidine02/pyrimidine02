<p align="center">
  <img src="https://r2.pyrimidines.org/readme/hero.png" alt="main character" width="260" />
</p>

<h1 align="center">손호영</h1>

<p align="center">
  <em>좋아하는 것들을 더 깊이 즐기기 위해 직접 만드는 개발자</em>
</p>

<p align="center">
  🎵 음악&nbsp;&nbsp;·&nbsp;&nbsp;🎸 기타&nbsp;&nbsp;·&nbsp;&nbsp;✈️ 라이브 원정&nbsp;&nbsp;·&nbsp;&nbsp;💻 백엔드 개발
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Kotlin-%237F52FF?style=flat-square&logo=kotlin&logoColor=white" />
  <img src="https://img.shields.io/badge/Spring_Boot-%236DB33F?style=flat-square&logo=springboot&logoColor=white" />
  <img src="https://img.shields.io/badge/PostgreSQL-%234169E1?style=flat-square&logo=postgresql&logoColor=white" />
  <img src="https://img.shields.io/badge/Redis-%23FF4438?style=flat-square&logo=redis&logoColor=white" />
</p>

---

## 🩵 About Me

현장에서 좋아하는 음악을 듣고, 그 경험을 기록하고, 더 좋은 서비스로 만드는 사이클을 즐깁니다.

팬으로서 "이런 기능이 있으면 좋겠다"는 생각이 드는 순간 직접 만들기 시작하는 타입입니다.
서브컬처·밴드 음악 커뮤니티가 교차하는 지점에서 사람들에게 실제로 쓰이는 서비스를 만들고 싶습니다.

```
좋아하는 것 → 부족함을 느낌 → 직접 만듦 → 더 깊이 즐김 → 반복
```

---

## 🛠️ Projects

### 🎸 Girls Band Tabi
> 밴드 음악 팬을 위한 위치 기반 소셜 플랫폼

`Spring Boot 3` · `Spring Modulith` · `PostgreSQL` · `Redis`

라이브 현장에서 느낀 경험을 기록하고, 같은 취향의 팬들과 공유하는 서비스입니다.
259개의 REST 엔드포인트, 80% 코드 커버리지, 실제 운영을 목표로 개발 중입니다.

---

## 🎀 Favorite Works & Characters

### 🎸 뱅드림!

<p align="center">
  <img src="https://r2.pyrimidines.org/readme/ranna.webp" alt="카나메 라나" width="160" />
  <img src="https://r2.pyrimidines.org/readme/aya.webp" alt="마루야마 아야" width="160" />
  <img src="https://r2.pyrimidines.org/readme/moka.webp" alt="아오바 모카" width="160" />
  <img src="https://r2.pyrimidines.org/readme/rock.webp" alt="아사히 롯카" width="160" />
  <img src="https://r2.pyrimidines.org/readme/arisa.webp" alt="이치가야 아리사" width="160" />
</p>
<p align="center">카나메 라나 · 마루야마 아야 · 아오바 모카 · 아사히 롯카 · 이치가야 아리사</p>

---

### 🥁 걸즈밴드크라이 · 🍷 데레마스 · 🎓 학원마스 · 🎼 결속밴드 · 🌟 Liella!

<p align="center">
  <img src="https://r2.pyrimidines.org/readme/subaru.webp" alt="아와 스바루" width="170" />
  <img src="https://r2.pyrimidines.org/readme/kaede.webp" alt="타카가키 카에데" width="170" />
  <img src="https://r2.pyrimidines.org/readme/kotone.webp" alt="후지타 코토네" width="170" />
  <img src="https://r2.pyrimidines.org/readme/ryo.webp" alt="야마다 료" width="170" />
  <img src="https://r2.pyrimidines.org/readme/keke.webp" alt="탕 쿠쿠" width="170" />
</p>
<p align="center">
  아와 스바루 · 타카가키 카에데 · 후지타 코토네 · 야마다 료 · 탕 쿠쿠
</p>

---

## 🎤 Favorite Voice Actor

<p align="center">
  <img src="https://pbs.twimg.com/media/HCU0N75acAAuSLS?format=jpg&name=large" alt="아오키 히나" width="380" />
</p>
<p align="center"><b>아오키 히나</b></p>
